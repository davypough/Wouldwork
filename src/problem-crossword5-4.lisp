;;;; Filename: problem-crossword5-4.lisp
...
(define-query get-remaining-fields? ()
  (do (bind (used-fields $used-fields))
      (set-difference *sorted-field-names* $used-fields)))


(define-query get-remaining-words? ()
  (do (bind (used-words $used-words))
      (set-difference *sorted-words* $used-words)))
...
(define-action fill
    1
    (?field (get-remaining-fields?) ?word (get-remaining-words?))  ;?field&word (get-remaining-fields&words?))
    (let ($new-cross-fills)
      (declare (special $new-cross-fills))
              ...
              do (text $cross-field $new-cross-str)
                 (if (full-word $new-cross-str)
                   (do (setf $new-cross-word (intern $new-cross-str))
                       (if (gethash $new-cross-word *sorted-words-ht*)  ;(member $new-cross-word *sorted-words*)  ;otherwise leave dictionary filling
