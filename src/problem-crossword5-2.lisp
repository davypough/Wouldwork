...
(define-types
    field (compute (iter (for (field nil) in *field&word-instantiations*)
                         (collect field)))
    word (compute (iter (for (nil word) in *field&word-instantiations*)
                        (collect word))))
...
(defun dictionary-compatible ($new-cross-str)  ;eg, "A?R??" of length 5
  ...
  (iter (with len = (length $new-cross-str))
        (for chr in-sequence $new-cross-str)
        (for index from 0)
        (when (char/= chr #\?)
...
(define-action fill
    1
    (dot-products ?field field ?word word)
    (let ($new-cross-fills)
      (declare (special $new-cross-fills))
