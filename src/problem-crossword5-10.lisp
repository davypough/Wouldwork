;;;; Filename: problem-crossword5-10.lisp
...
;;; Skip over words for *trie-ht* that are too short or too long

(in-package :ww)


(ww-set *problem* crossword5-10)
                     ...
                     (loop for key being the hash-keys of node using (hash-value child-node)
                        do (when (hash-table-p child-node)
                             (let ((result (search-node child-node pattern
                                                        (concatenate 'string current-word (string key))
                                                        (1+ index))))
                               (when result
                                 (return result)))))
                     ;; regular character
                     (let ((child-node (gethash chr node)))
                       (when child-node
                         (search-node child-node pattern
                                      (concatenate 'string current-word (string chr))
                                      (1+ index)))))))))
                 ...
                 (let ((chr (char pattern index)))
                   (if (eql chr #\?)
                     ...
                     (loop for key being the hash-keys of node using (hash-value child-node)
                        ...
                        nconc (search-node child-node pattern
         ...
         (using ($cross-field $cross-index $word-index) on $crosscuts by #'cdddr
           always (do (bind (text $cross-field $cross-str))
                      ...
                      (if (string= $new-cross-str $cross-str)  ;no change?
                        t  ;returns true
                        (if t  ;(dictionary-compatible $new-cross-str)
...
#| ;old version
(defun feasible-solutions (field-sets)  ;use this second (feasible-solutions *)
  ;Finds puzzle solutions by filling in compatible dictionary words.
             ...
             (if (null field-sets)
               ...
               (let ((current-set (car field-sets)))
                 (loop for word in (cdr current-set)
                       ...
                       when (every #'identity
                                   (mapcar (lambda (x) (compatible-words field+word x))
                                           current-collection))
                       nconc (collector (cdr field-sets) (cons  field+word current-collection)))))))
...
#|
(defun feasible-solutions (field-sets)  ;tail recursion doesn't work
  (let ((memo (make-hash-table :test 'equal)))
    (labels ((collector (field-sets current-collection acc)
               (if (null field-sets)
                 (nreverse acc)
                 (let ((current-set (car field-sets))
                       (new-acc acc))
                   (loop for word in (cdr current-set)
                         ...
                         do (when (every #'identity
                                         (mapcar (lambda (x)
                                                 ...
                                                 current-collection))
                              (setf new-acc 
                                    (collector (cdr field-sets) 
                                               (cons field+word current-collection) 
                                               new-acc))))
                   new-acc))))
      (collector field-sets nil nil))))
|#