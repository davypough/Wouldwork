...
(defun process-threads ()
  ...
  (let ((channel (lparallel:make-channel))
    ...
    (iter (for i from 1 to *num-parallel-threads*)  ;get all threads up & running
      (lparallel:submit-task channel #'search-parallel problems done))
    ...
    (iter (for i from 1 to *num-parallel-threads*)
      (lparallel.queue:push-queue (hs::make-hstack) problems)))  ;send stop message to all threads
...
(defun split-off (open)
  ...
  (iter (with i = 1) (with j = 2)
        ...
        (when (= (node.depth (hs::nth-hstack i open)) (node.depth (hs::nth-hstack j open)))
          (let ((new-open #+:sbcl (hs::create-hstack :element-type '(or node null) 
                                                     :ht-keyfn #'node.state
                                                     :ht-test 'state-equal-p :synchronized t)
                          #+:allegro (hs::create-hstack :element-type '(or node null) 
                                                        :ht-keyfn #'node.state
