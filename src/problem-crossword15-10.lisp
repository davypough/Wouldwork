;;;; Filename: problem-crossword15-10.lisp
...
(ww-set *progress-reporting-interval* 50000)
...
(defparameter *words* '(ADMIRAL ALAN ANN AQUABELLES AQUA ARCH ASA BELLES BILL BONNIE ATTIC ATTICFAN
 AUDREY AUNT AUNTFRANK AUNTPAT AVE AVENUE BEAR BEEHIVE BEES BETTY BIGBOB BLACK BOB BRER
 ...
 FOOTEAVE FOREST FOX FRANK FRED FREDDIE GARDENS GEORGE GEORGIE GRACE GRACEAVE GRAMMY GROVES ICERINK INDIANA JAMIE
 ...
 PAMLICO PAT PAPAW PARK PERSIMMON PIANO PENOCHLE POLLY QUEENIE RABBIT RED REDCROSS
 REMUS REP RICHARD ROCKHILL SCHOOL SCRABBLE SCRUGGS
 SHIRLEY SIESTA SIESTAKEY SISTER SKATING SKIPPY STEVE SUE SUGAR TENNESSEE TOM TOMMY TROUT UNCLE
             ...
             (if (= index (length pattern))
                 ...
                 (let ((chr (char pattern index)))
                   (if (eql chr #\?)
                     ;; wildcard character
                     (loop for key being the hash-keys of node using (hash-value child-node)
                           when (hash-table-p child-node)  ;not :word leaf
                           append (let ((new-current-word 
                                           (make-array (length current-word) :element-type 'character
                                                       :initial-contents current-word :adjustable t
                                                       :fill-pointer (fill-pointer current-word))))
                                     (vector-push-extend key new-current-word) 
                                     (1+ index)
                                     (list new-current-word)))
                     ;; regular character
                     (let ((child-node (gethash chr node)))
                       (when child-node
                         (search-node child-node pattern
