...
#+:sbcl (defparameter *visited*
          (if (eql *tree-or-graph* 'graph)
            (if (> *num-parallel-threads* 0)
              (make-hash-table :size *max-states* :test 'state-equal-p :synchronized t)
              (make-hash-table :size *max-states* :test 'state-equal-p))
...
(defun summarize-search-results (condition)
  ...
  (when (eql *tree-or-graph* 'graph)
    (format t "~2%Repeated states = ~:D, ie, ~F percent"
