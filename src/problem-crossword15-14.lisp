;;;; Filename: problem-crossword15-14.lisp
...
;;; *Pre-instantiation of equal length field & word, ?field.word
;;; Representing words initially as strings rather than symbols, "LIZ" vs LIZ
;;; Generate pre-instantiations dynamically from get-remaining-field.word-strings?

(in-package :ww)


(ww-set *problem* crossword15-14)
...
(define-dynamic-relations
  ...
  (remaining-field.word-strings $hash-table))
...
(define-query get-remaining-field.word-strings? ()
  (do (bind (remaining-field.word-strings $remaining-field.word-strings))
      (alexandria:hash-table-keys $remaining-field.word-strings)))
...
(define-query crosscuts-compatible? ($word-string $field $used-fields)  
  (let ()
...
(define-action fill
    1
    (?field.word-string (get-remaining-field.word-strings?))  ;list of remaining (FIELD1 . "LIZ") pairs
    (let ($new-cross-fills)
      (declare (special $new-cross-fills))
      (and (multiple-value-setq ($field $word-string) (values (car ?field.word-string) (cdr ?field.word-string)))
           (word-compatible? $word-string $field)
           (bind (used-fields $used-fields))
           (crosscuts-compatible? $word-string $field $used-fields)))
    (?field.word-string (get-remaining-field.word-strings?))
    (assert (bind (remaining-field.word-strings $remaining-field.word-strings)) 
            (setf $new-remaining-field.word-strings (alexandria:copy-hash-table $remaining-field.word-strings))
            (bind (used-fields $used-fields))
            ...
            (maphash (lambda (field.word-string value)
                       (declare (ignore value))
                       (when (or (eq (car field.word-string) $field)
                                 (string= (cdr field.word-string) $word-string))
                         (remhash field.word-string $new-remaining-field.word-strings)))
                     $new-remaining-field.word-strings)
                 ...
                 (if (full-word $new-cross-str)
                   (do ;(setf $new-cross-word (intern $new-cross-str))
                       (if (gethash $new-cross-str *word-strings-ht*)  ;otherwise leave dictionary filling
                         (do (maphash (lambda (field.word-string value)
                                        (declare (ignore value))
                                        (when (or (eq (car field.word-string) $cross-field)
                                                  (string= (cdr field.word-string) $new-cross-string))
                                          (remhash field.word-string $new-remaining-field.word-strings)))
                                      $new-remaining-field.word-strings)
                             (setf (gethash $cross-field $new-used-fields) t))))))
            (remaining-field.word-strings $new-remaining-field.word-strings)
...
(define-init
  `(used-fields ,(make-hash-table :test #'eq))  ;eg, FIELD1 => T
  `(remaining-field.word-strings ,(let ((ht (make-hash-table :test #'equal)))  ;eg, (FIELD1 "LIZ") => T
                                    (iter (for (field field-length) in *sorted-fields*)
                                          (iter (for (word-string nil) in-hashtable *word-strings-ht*)
                                                (when (= field-length (length word-string))
                                                  (setf (gethash (cons field word-string) ht) t)))
                                          (finally (return ht))))))


(define-init-action initialize-database
  0
  ()
  (always-true)
  ()
  (do (assert (using ($field $field-cuts) in *crosscuts*
