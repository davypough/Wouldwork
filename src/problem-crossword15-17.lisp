;;;; Filename: problem-crossword15-17.lisp
...
;;; Manual staged search with intermediate best-state becoming next start-state
...
(ww-set *progress-reporting-interval* 20000)
...
(defparameter *fields-ht*
  (let ((ht (make-hash-table :test #'eq)))
    (iter (for (field len) in *sorted-fields*)
          (when (>= len 8)
...
(define-query get-remaining-word-strings? ()
  (do (bind (used-word-strings-ht $used-word-strings-ht))
      (sort (ut::ht-set-difference *word-strings-ht* $used-word-strings-ht)
            (lambda (x y)
              (> (length x) (length y))))))
           ...
           always (do (bind (text $cross-field $cross-str))
                      ...
                      (if (char= $cross-char #\?)
                        (do (setf $new-cross-str  ;replace one letter from word-string into cross-str
                            ...
                            (dictionary-compatible $new-cross-str))
...
(define-action fill
    ...
    ;(let ($new-cross-fills)
      ...
      (and (word-compatible? ?word-string ?field)
           ...
           (crosscuts-compatible? ?word-string ?field))
    (?field (get-remaining-fields?) ?word-string (get-remaining-word-strings?))
    (assert (bind (used-word-strings-ht $used-word-strings-ht))
            ...
            ;(setf $objective-value (iter (for (key nil) in-hashtable $new-used-word-strings-ht)
            ;                             (sum (length key))))))
            (setf $objective-value (hash-table-count $new-used-fields-ht))))
...
(define-init
  `(used-fields-ht ,(make-hash-table :test #'eq))  ;remove these after first run
...
(define-init-action initialize-crosscuts&text
  ...
  (assert (using ($field $field-cuts) in *crosscuts*
          ...
          (using ($field $field-length) in *sorted-fields*  ;remove this after first run
