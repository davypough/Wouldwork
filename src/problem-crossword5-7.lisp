;;;; Filename: problem-crossword5-7.lisp
...
(ww-set *problem* crossword5-7)


(ww-set *tree-or-graph* graph)


(ww-set *solution-type* max-value)  ;maximize number of used fields
...
(defun trie-search (trie pattern)
  ...
  (labels ((search-node (node pattern current-word)
             (if (null pattern)
                   ...
                   (if (eql chr #\?)
                     ...
                     (loop for key being the hash-keys of node using (hash-value child-node)
                        do (when (hash-table-p child-node)
                             (let ((result (search-node child-node rest-pattern
                                                        (append current-word (list key)))))
                               (when result
                                 (return result)))))
                     ;; regular character
                 ...
                 (let ((chr (car pattern))
                   ...
                   (if (eql chr #\?)
                     ...
                     (loop for key being the hash-keys of node using (hash-value child-node)
                        ...
                        append (search-node child-node rest-pattern
                                            (append current-word (list key))))
                     ;; regular character
                     (let ((child-node (gethash chr node)))
                       (when child-node
...
(defun dictionary-compatible-all ($new-cross-str)  ;eg, "A?R??" of length 5
  ...
  (trie-search-all *trie-ht* (coerce $new-cross-str 'list)))
