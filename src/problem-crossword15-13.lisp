;;;; Filename: problem-crossword15-13.lisp
...
(define-types
  field.word-string (compute (iter (for (field field-length) in *sorted-fields*)
                            (nconcing (iter (for (word-string nil) in-hashtable *word-strings-ht*)
                                            (when (= field-length (length word-string))
                                              (collect (cons field word-string)))))))
  field (compute *sorted-field-names*))


(define-dynamic-relations
  (used-fields $hash-table)
  (used-word-strings $hash-table)
  (text field $string))
...
(define-action fill
    1
    (?field.word-string field.word-string)  ;(?field (get-remaining-fields?) ?word (get-remaining-words?))
    (let ($new-cross-fills)
      (declare (special $new-cross-fills))
    ...
    (?field.word-string field.word-string)  ;(?field field ?word word)
    (assert (bind (used-word-strings $used-word-strings)) 
            (setf $new-used-word-strings (alexandria:copy-hash-table $used-word-strings))
            (bind (used-fields $used-fields))
            ...
            (setf (gethash $word-string $new-used-word-strings) t)
                 ...
                 (if (full-word $new-cross-str)
                   (do ;(setf $new-cross-word (intern $new-cross-str))
                       (if (gethash $new-cross-str *word-strings-ht*)  ;otherwise leave dictionary filling
                         (do (setf (gethash $new-cross-str $new-used-word-strings) t)
                             (setf (gethash $cross-field $new-used-fields) t))))))
            (used-word-strings $new-used-word-strings)
...
(define-init
  ...
  `(used-word-strings ,(make-hash-table :test #'equal)))
