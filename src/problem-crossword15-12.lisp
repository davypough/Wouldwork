;;;; Filename: problem-crossword15-12.lisp
...
(define-types
  field.word (compute (iter (for (field field-length) in *sorted-fields*)
                            (nconcing (iter (for word in *sorted-words*)
                                            (when (= field-length (length (string word)))
                                              (collect (cons field word)))))))
...
(define-action fill
    1
    (?field.word field.word)  ;(?field (get-remaining-fields?) ?word (get-remaining-words?))
    (let ($new-cross-fills)
      (declare (special $new-cross-fills))
      (and (multiple-value-setq ($field $word) (values (car ?field.word) (cdr ?field.word)))
           (word-compatible? $word $field)
           (bind (used-fields $used-fields))
           (crosscuts-compatible? $word $field $used-fields)))
    (?field.word field.word)  ;(?field field ?word word)
    (assert (bind (used-words $used-words)) 
            ...
            ;(setf (gethash $field-id $new-used-fields) t)
