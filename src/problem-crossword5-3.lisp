;;;; Filename: problem-crossword5-3.lisp

;;; Problem specification for 5x5 crossword, best filling

;;; This is the second version of crossword5.
;;; It generates instantiations by
;;; pairing fields and words of equal length, in order of descending length
;;; instead of all possible fields & words.
;;; Also adds dynamic instantiations with get-remaining-fields&words.


(in-package :ww)


(ww-set *problem* crossword5-3)
...
(defparameter *field&word-instantiations*
;Generate initial instantiations for the fill action rule
  (iter (for (field field-length) in *sorted-fields*)
        (nconcing (iter (for word in *sorted-words*)
                        (when (= field-length (length (string word)))
                          (collect (list field word)))))))
...
(define-query get-remaining-fields&words? ()
  (do (bind (remaining-fields&words $remaining-fields&words))
      $remaining-fields&words))
...
(defun dictionary-compatible ($new-cross-str)  ;eg, "A?R??" of length 5
  ...
  (iter (with len = (length $new-cross-str))
        ...
        (for index from 0)
        (when (char/= char #\?)
          (collect (gethash (format nil "~D~C~D" len char index)
...
(define-action fill
    1
    (?field&word (get-remaining-fields&words?))
    (let ($new-cross-fills)
      (declare (special $new-cross-fills))
      (and (setf $field (first ?field&word))
           (setf $word (second ?field&word))
    ...
    (assert (bind (remaining-fields&words $remaining-fields&words))
            (setf $new-remaining-fields&words (copy-list $remaining-fields&words))
            (setf $new-used-words (copy-list $used-words))
            ...
            (setf $new-remaining-fields&words
                  (delete-if (lambda (pair)
                               (or (eq (first pair) $field)
                                   (eq (second pair) $word)))
                             $new-remaining-fields&words))
            ...
            (remaining-fields&words $new-remaining-fields&words)
...
(define-init
  (used-fields nil)
  (used-words nil)
  `(remaining-fields&words ,*field&word-instantiations*))
